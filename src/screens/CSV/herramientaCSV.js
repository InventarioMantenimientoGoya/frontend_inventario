...
import { View, Text, ImageBackground, StyleSheet, Pressable, Platform, ScrollView, Modal, Image, Alert } from 'react-native';
...
const HerramientaCSV = () => {
    ...
    const GuardarHerramientasCSV = async () => {

        // Se muestra el modal de progreso para indicar que el proceso de guardado está en curso
        setIsModalProgresoVisible(true);

        const url = urlApi + 'herramientas/'; // Se define la URL de la API donde se enviarán los datos
        ...
        datosCorrectos.map((herramienta) => {

            // Se validan los campos numéricos, y si alguno no es válido, se muestra un modal de error
            if (
                !esNumeroValido(herramienta["Stock Inicial"]) ||
                !esNumeroValido(herramienta["Stock Mínimo"]) ||
                !esNumeroValido(herramienta["Stock"]) ||
                (herramienta['Costo Unitario'] && !esNumeroValido(herramienta['Costo Unitario'])) ||
                (herramienta['Costo Total'] && !esNumeroValido(herramienta['Costo Total']))
            ) {
            ...
            const categoriaEncontrada = categoriasDB.find((categoria) => categoria.Cat_nombre === herramienta["Categoría"]);
            ...
            const herramientaBody = {
                "Her_nombre": herramienta["Nombre"],
                "Her_caracteristica1": herramienta["Carácteristica 1"],
                "Her_caracteristica2": herramienta["Carácteristica 2"] ? herramienta["Carácteristica 2"] : undefined,
                "Her_marca": herramienta["Marca"],
                "Her_stock_inicial": parseFloat(herramienta["Stock Inicial"]),
                "Her_unidad_medida": herramienta["Unidad Medida"],
                "Her_ubicacion": herramienta["Ubicación"],
                "Her_stock_minimo": parseFloat(herramienta["Stock Mínimo"]),
                "Her_stock": parseFloat(herramienta["Stock"]),
                "Her_costo_unitario": herramienta['Costo Unitario'] ? Number(herramienta['Costo Unitario']) : undefined,
                "Her_costo_total": herramienta['Costo Total'] ? Number(herramienta['Costo Total']) : undefined,
                "Her_observaciones": herramienta["Observaciones"],
                "Her_categoria": codigoCategoria
            };

            // Se eliminan las propiedades con valores undefined del objeto repuestoBody
            Object.keys(herramientaBody).forEach(key => {
                if (herramientaBody[key] === undefined) {
                    delete herramientaBody[key];
                }
            });

            // Se agrega el repuesto a la lista de repuestos que serán enviados
            body.push(herramientaBody);
        });
        ...
        try {
            ...
            if (response.status !== 201) {
                ...
                setIsModalProgresoVisible(false); // Se oculta el modal de progreso
                Alert.alert("¡Registro de herramientas exitoso!", "Los datos se han registrado correctamente.");
        ...
        >
            ...
            <View style={styles.centeredContainer}>
                <View style={styles.card}>
                    {/* Título Principal */}
                    <Text style={styles.title}>Subir Herramientas</Text>
                    ...
                    {archivoImportado ? (
                        <View style={{ flexShrink: 1, alignItems: 'center', width: '100%' }}>
                            ...
                            <Pressable style={styles.subirButton} onPress={GuardarHerramientasCSV}>
                        ...
                        <>
                            ...
                            <Text style={styles.description}>
                                Por favor, seleccione y cargue un archivo <Text style={FONTS.boldText}>CSV</Text> en formato <Text style={FONTS.boldText}>UTF-8</Text> para importar las herramientas correctamente.
                            </Text>
...
const styles = StyleSheet.create({
    ...
    card: { backgroundColor: 'rgba(255, 255, 255, 0.9)', borderRadius: 15, padding: 25, paddingHorizontal: 15, alignItems: 'center', width: '85%', overflow: 'hidden', },
    // Título Principal
