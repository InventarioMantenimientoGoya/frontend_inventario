...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, Image, Alert, ImageBackground, Pressable, Modal } from "react-native";
...
const VerificacionLogin = ({ route }) => {

    // Obtener usuarioCorreo desde los parámetros
    const { usuarioCorreo } = route.params;

    const { usuarioContrasena } = route.params;
    ...
    const correoVerificado = Transmision.decrypt(usuarioCorreo);



    // Simulando el correo que viene del login o props
    const correoUsuario = correoVerificado;
    ...
    const handleVerify = async () => {
        ...
        // Código correcto, cancelamos el timer y navegamos
        clearInterval(intervalRef.current);

        try {

            const contrasenaVerificada = Transmision.decrypt(usuarioContrasena);

            const { token, usuario: usuarioLogueado } = await AuthService.login(correoVerificado, contrasenaVerificada);
            ...
            // Puedes mostrar un modal de error general aquí si quieres
            setIsModalCargaVisible(true);
        }
    ...
    return (
        <ImageBackground
            source={require('../../../assets/img/Img_10.jpg')}
...
export default VerificacionLogin;
